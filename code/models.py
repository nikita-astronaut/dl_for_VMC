...
def _conv_2d_model(x, weights, biases):
    print(x)
    x = periodic_padding(x, padding = (weights['w_conv1'].get_shape().as_list()[1]) // 2)
    print(x)
    conv1 = conv2d(x, weights['w_conv1'], biases['b_conv1'])
    print(conv1)
    # conv1 = periodic_padding(conv1, padding = (weights['w_conv2'].get_shape().as_list()[1]) // 2)
    ...
    # conv2 = periodic_padding(conv2, padding = (weights['w_conv3'].get_shape().as_list()[1]) // 2)
    # Max Pooling (down-sampling), this chooses the max value from a 2*2 matrix window and outputs a 7*7 matrix.
    # conv2 = maxpool2d(conv2, k=2)

    # conv3 = conv2d(conv2, weights['w_conv3'], biases['b_conv3'])
    # conv3 = periodic_padding(conv3, padding = (weights['w_conv4'].get_shape().as_list()[1]) // 2)
    # conv4 = conv2d(conv3, weights['w_conv4'], biases['b_conv4'])
    # Max Pooling (down-sampling), this chooses the max value from a 2*2 matrix window and outputs a 4*4.
    # conv3 = maxpool2d(conv3, k=2)
    # conv4 = maxpool2d(conv1, k=conv1.shape[1])
    conv4 = tf.layers.average_pooling2d(conv1, pool_size=(3, 3), strides=(1,1))
    ...
    # out[:, 0] = np.pi * tf.sigmoid(out[:, 0])
    return out

def _conv_2d_model_debug(x, weights, biases):
    print(x)
    x = periodic_padding(x, padding = (weights['w_conv1'].get_shape().as_list()[1]) // 2)
    print(x)
    conv1 = conv2d(x, weights['w_conv1'], biases['b_conv1'])
    ...
    out = tf.add(tf.matmul(fc1, weights['out']), biases['out'])
    # print(out.shape, fc1.shape)
    return out, x, conv1, conv2, conv3, conv4, fc1
...
def conv2d_model(x_bra, x_ket, input_shape):
    ...
    weights = {
        ...
        # 'w_conv2': tf.get_variable('W1', shape=(5,5,32,64), initializer=tf.contrib.layers.xavier_initializer()),
        ...
        'w_dense1': tf.get_variable('W4', shape=(8, 8), initializer=tf.contrib.layers.xavier_initializer()), 
        'out': tf.get_variable('W6', shape=(8,2), initializer=tf.contrib.layers.xavier_initializer()), 
    }
    ...
    biases = {
        ...
        'b_dense1': tf.get_variable('B4', shape=(8), initializer=tf.contrib.layers.xavier_initializer()),
